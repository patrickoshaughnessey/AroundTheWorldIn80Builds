...
if(global.Sequence && Sequence.delivery){
    if(!script.allowTapDelivery && !global.deviceInfoSystem.isEditor()) script.allowTap = false;
}
...
const pinchThreshold = (script.allowSIK && global.deviceInfoSystem.isSpectacles()) ? 1.8 : 3.3; // thumb & index distance threshold for hand tracking, cm (SIK is more precise)
...
function startSIK(){
    // get module
    var SIK = require("Spectacles Sync Framework/SpectaclesInteractionKit/SIK").SIK;
    ...
    function pinchingUpdate(){
        ...
        // check if pinching
        isPinching = SIKPinchDetectManually ? thumbPos.distance(indexPos) < pinchThreshold : activeHandObject.isPinching();
